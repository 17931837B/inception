...
**【クイックスタート】**
プロジェクトを起動するには、リポジトリのルートディレクトリで以下のコマンドを実行します。
`make all`

## 3. Resources (リソース)

本プロジェクトの開発にあたり、以下のリソースを活用しました。

* Docker 公式ドキュメント
* NGINX, WordPress, MariaDB 公式ドキュメント
* **AIの使用について**: 生成AIは、主にエラーログの解析、設定ファイル（NGINXのルーティング設定やDockerのボリュームマウントパスなど）のデバッグ、および評価要件を満たすためのMarkdownドキュメントの構成・翻訳サポートとして使用しました。生成されたコードや構成はすべて手動でレビューし、動作テストを行い、仕組みを完全に理解した上で実装しています。
...
* **本プロジェクトでの採用:**
    カスタムブリッジネットワーク `inception_network`（またはデフォルトのプロジェクトネットワーク）を作成しています。これにより、MariaDB を外部から隠蔽し、WordPress からのみアクセス可能にする安全な構成を実現しています。
